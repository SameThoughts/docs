...
1. **Configure memory.** Pass file paths to `memory=` and optional [skills](/oss/javascript/deepagents/skills) to `skills=`. A [backend](/oss/javascript/deepagents/backends) controls storage and access.
2. **Agent reads memory.** Files load into the system prompt at startup, or on demand during the conversation.
3. **Agent writes memory.** The agent updates files via `edit_file` as it learns. Some memory can be [read-only](#read-only-vs-writable-memory).

[Skills](/oss/javascript/deepagents/skills) are procedural memory: reusable instructions that tell the agent *how* to perform a task. They use on-demand loading — the agent reads only skill descriptions at startup, then loads the full skill when it matches a task. This keeps context lean until a capability is needed. Developer-defined skills and [organization policies](#organization-level-memory) are typically not writable by the agent. See [read-only vs writable memory](#read-only-vs-writable-memory).
